...
class PII:
    ...
    def findNM2(rowMatchingLookUp, columnMatchingLookUp, preferenceStructure, nm1, ITER):
        ...
        for ii in range(0, len(nm2GeneratingIndices)):
            ...
            #the mn2 generating pair is just a row end
            if (rowEnd == 1) and (colEnd == 0):

                if (len(columnEnds) > 0):
                    nm2pair = [columnEnds[0], currColumnIndex]
                    del columnEnds[0]
                    nm2Pairs.append(nm2pair)
                else:
            ...
            #the nm2 generating pair is just a column end
                ...
                if (len(rowEnds) > 0):
                    nm2Pair = [currRowIndex, rowEnds[0]] #need row value from column end and column value from row end
                    del rowEnds[0]
                    nm2Pairs.append(nm2Pair)
                else:
    ...
    def PII(preferenceStructure, n, initialMatching):
        ...
        while (not stable):
            ...
            if ITER < n:
                nm1Pairs = PII.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
            elif ITER > n and ITER < 3*n:
                nm1Pairs = Improved.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
                if (len(nm1Pairs) <= 0):
                    nm1Pairs = PII.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
                    if (len(nm1Pairs) > 0 and (not broken)):
                        nm1Pairs = Improved.breakMarraige(preferenceRDD, columnMatchingLookUp, preferenceStructure)
                        broken = True
            elif nm1PairsFound == True:
                nm1Pairs = tempNm1pairs
            else:
                nm1Pairs = PII.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
            ...
            if (len(nm1Pairs) <= 0):
                ...
                if ITER >= 3*n and not nm1PairsFound:             #implementing Colin's code
                    tempNm1pairs = cycleDetection.nm1Cycle(nm1Pairs, ITER, n, rowMatchingLookUp)
                    if len(tempNm1pairs) != 0:
                        nm1PairsFound = True
...
'''class vanillaPII:
    ...
    def PII(preferenceStructure, n, initialMatching):
        ...
        while (not stable):
            nm1Pairs = vanillaPII.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
            #updates CHANGE BACK
            ...
            if (len(nm1Pairs) <= 0):
                ...
                nm2Pairs = vanillaPII.findNM2(rowMatchingLookUp, columnMatchingLookUp, preferenceStructure, nm1Pairs, ITER)
                [rowMatchingLookUp, columnMatchingLookUp] = vanillaPII.updateMatching(rowMatchingLookUp, columnMatchingLookUp, preferenceStructure, nm1Pairs, nm2Pairs)
                ITER += 1
                #*****TESTING*****
                if (Detector2.detectCycle(ITER, n, rowMatchingLookUp, preferenceStructure)):
                    Convergence.logIterations(ITER)
...
class nm2PII:
    ...
    def PII(preferenceStructure, n, initialMatching):
        ...
        while (not stable):
            ...
            nm1Pairs = nm2PII.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
            ...
            if (len(nm1Pairs) <= 0):
                ...
                nm2Pairs = nm2PII.findNM2(rowMatchingLookUp, columnMatchingLookUp, preferenceStructure, nm1Pairs, ITER)
                [rowMatchingLookUp, columnMatchingLookUp] = nm2PII.updateMatching(rowMatchingLookUp, columnMatchingLookUp, preferenceStructure, nm1Pairs, nm2Pairs)
                ITER += 1
                #*****TESTING*****
...
class nm1nm2PII:
    ...
    def PII(preferenceStructure, n, initialMatching):
        ...
        while (not stable):
            ...
            if ITER < n:
                nm1Pairs = nm1nm2PII.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
            else:
                nm1Pairs = Improved.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
                if (len(nm1Pairs) <= 0):
                    nm1Pairs = PII.findNMI(preferenceRDD, columnMatchingLookUp, preferenceStructure)
            ...
            if (len(nm1Pairs) <= 0):
                ...
                nm2Pairs = nm1nm2PII.findNM2(rowMatchingLookUp, columnMatchingLookUp, preferenceStructure, nm1Pairs, ITER)
                [rowMatchingLookUp, columnMatchingLookUp] = nm1nm2PII.updateMatching(rowMatchingLookUp, columnMatchingLookUp, preferenceStructure, nm1Pairs, nm2Pairs)
                ITER += 1
                #*****TESTING*****
                #print("Matching in Algo = " + str(rowMatchingLookUp))
                if (Detector2.detectCycle(ITER, n, rowMatchingLookUp, preferenceStructure)):
